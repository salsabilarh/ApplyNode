...
import { BriefcaseBusiness, User, Mail, Lock, Loader2, Eye, EyeOff } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect to dashboard/home because token is already stored via cookie
      router.push('/jobs'); 
      router.refresh();
    } catch (err: any) {
    ...
    <div className="w-full min-h-screen flex items-center justify-center bg-slate-50 px-4 py-6 select-none">
      <div className="max-w-md w-full bg-white p-6 md:p-8 rounded-2xl border border-slate-200/60 shadow-[0_4px_12px_rgba(0,0,0,0.02)]">
        ...
        <div className="flex flex-col items-center text-center mb-6">
          ...
          <p className="text-xs text-slate-400 mt-1">Register to start tracking your recruitment progress.</p>
        </div>
